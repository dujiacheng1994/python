...
#Todo:此处要分两次运行！
criterion = nn.CrossEntropyLoss()  #这也是个module有自己的参数什么的
...
for epoch in range(10):
    ...
    for i, data in enumerate(trainloader, 0):  # 适用于没有len的情况，如迭代器，生成器。 从0开始，每个trainloader的对象x[i]会变成（i,x[i]）
        inputs, labels = data
        optimizer.zero_grad()
        outputs = net(inputs) #正向forward，y=f(x)
        loss = criterion(outputs, labels)  #todo:这怎么解释？
        loss.backward()  #todo:网络未知，怎么反向传播的，net.parameters()指定了net！
        optimizer.step()  #改变net.parameters

        running_loss += loss.item()
        if i % 2000 == 1999:       #train一共8000,batchsize为4,故2000轮
