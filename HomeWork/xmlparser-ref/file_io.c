...
XMLParserContext *read_XML_file(char *in)
{
	...
	fseek(h->XMLfilein, 0, SEEK_END);       //文件指针移到末尾
	h->XMLlength = ftell(h->XMLfilein);     //配合上句，得到文件位置指针当前位置相对于文件首的偏移字节数（即文件长度）
	fseek(h->XMLfilein, 0, SEEK_SET);       //文件指针移到开头
	h->XMLbuf = (char *)malloc(sizeof(char) * h->XMLlength);    //文件作为整体读取到XMLbuf中
