...
try:
    ...
    df=[]
    while sheet < 6:
        df.append(pd.read_excel(r'C:\Users\Administrator\Desktop\456.xlsx',sheet_name=sheet))
        sheet = sheet + 1

    f = open(r'urls.txt', "a", encoding='utf-8-sig')
    # sql = "select ifnull(rr.qq,1) as a from (select max(id) as qq from `picbook`) rr"  #获取当前绘本起始id  #todo: picbookid写到excel里，或者发布时将id作为参数
    # cur.execute(sql)
    # base_picbookid = cur.fetchall()[0][0]
    base_picbookid = 2    #todo:每次记得修改

    sql = "select ifnull(tt.pp,0) as b from (select max(id) as pp from `page`) tt"   #获取当前页面起始id
    cur.execute(sql)
    base_pageid = cur.fetchall()[0][0]
    sql = "select ifnull(ll.nn,0) as c from (select max(id) as nn from `image`) ll"  #获取当前图片起始id
    cur.execute(sql)
    base_imageid = cur.fetchall()[0][0]
    sql = "select ifnull(ll.nn,0) as c from (select max(id) as nn from `role`) ll"  #获取当前角色起始id
    cur.execute(sql)
    ...
    for m in range(len(df[2])):   #基于角色个数循环
        ...
        if len(df[4]) != 0:  # 若解释图不为空，插入解释图 todo:以后该循环要拿出来，当会出现无法一一对应的情况时
            imageid = imageid + 1   #Todo: 第一幅图介绍要改为卡，jpg
    ...
    if len(df[4]) == 0:
        # 否则插入对话图
        for i in range(len(df[5])):  # TODO 规范循环变量
            imageid = imageid + 1
